...
if __name__ == '__main__':
    ...
    group_id = sys.argv[7]
    sns_success_arn = sys.argv[8]
    non_anonymized_bucket = sys.argv[9]
    non_anonymized_db = sys.argv[10]
    ...
    try:
        ...
        send_notification(sns_arn, 'initial', err, 'Getting etl job config', group_id)
    ...
    while i < config_df.count():
        table_info = {
            ...
            'non_hudi_mode': True if str(config_df.select('non_hudi_mode').collect()[i][0]).strip().lower() == 'true' else False,
            ...
            'source_partition_to_col': True if str(config_df.select('source_partition_to_col').collect()[i][0]).strip().lower() == 'true' else False,
    ...
    write_logs_to_table(sparksession, write_logs_df, log_history_s3_path, sns_arn, sns_success_arn, group_id)
